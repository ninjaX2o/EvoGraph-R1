...
class Worker(Worker):
    ...
    def init_device(self) -> None:
        if self.device_config.device.type == "cuda":
            ...
            raw_local_rank = int(os.getenv("LOCAL_RANK", "0"))
            visible_device_count = torch.cuda.device_count()
            if visible_device_count <= 0:
                raise RuntimeError("No visible CUDA device for vLLM worker.")
            local_rank = raw_local_rank % visible_device_count
            if local_rank != raw_local_rank:
                print(
                    f"[verl-vllm] remap LOCAL_RANK={raw_local_rank} to cuda:{local_rank}; "
                    f"visible_cuda_devices={visible_device_count}, "
