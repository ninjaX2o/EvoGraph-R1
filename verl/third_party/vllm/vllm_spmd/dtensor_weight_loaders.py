...
def qwen2_5_vl_dtensor_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    stacked_params_mapping = [
        # (param_name, shard_name, shard_id)
        ("qkv_proj", "q_proj", "q"),
        ("qkv_proj", "k_proj", "k"),
    ...
    for name, loaded_weight in actor_weights.items():
        ...
        # Qwen2.5-VL keeps the vision MLP gate/up projections separate in vLLM,
        # while the language model still uses merged gate_up/qkv parameters.
...
__MODEL_DTENSOR_WEIGHT_LOADER_REGISTRY__ = {
    ...
    "Qwen2_5_VLForConditionalGeneration": qwen2_5_vl_dtensor_weight_loader,
}
