...
def _load_image_processor(name_or_path):
    if not name_or_path:
        return None
    try:
...
def _normalize_vllm_hf_config(model_hf_config):
    """Use vLLM's config class for Qwen2-VL profiling on vLLM 0.6.x."""
    if getattr(model_hf_config, "model_type", None) != "qwen2_vl":
        return model_hf_config
    if model_hf_config.__class__.__module__.startswith("vllm."):
        return model_hf_config
    try:
        from vllm.transformers_utils.configs.qwen2vl import Qwen2VLConfig as VllmQwen2VLConfig
    except Exception:
        return model_hf_config
    config_dict = model_hf_config.to_dict()
    ...
    return VllmQwen2VLConfig.from_dict(config_dict)
...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        model_hf_config = _normalize_vllm_hf_config(model_hf_config)
        ...
        self.image_processor = _load_image_processor(getattr(tokenizer, "name_or_path", None))
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        non_tensor_batch = self._align_non_tensor_batch(
    ...
    @staticmethod
    def _align_non_tensor_batch(non_tensor_batch, batch_size: int):
        ...
        for key, value in non_tensor_batch.items():
            ...
            if length == batch_size:
                ...
                aligned[key] = np.array([], dtype=object)
            elif batch_size % length == 0:
                aligned[key] = np.repeat(array, batch_size // length, axis=0)
            else:
                repeats = int(np.ceil(batch_size / length))
                aligned[key] = np.tile(array, (repeats,) + (1,) * (array.ndim - 1))[:batch_size]
        return aligned
