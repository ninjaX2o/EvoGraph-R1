...
def _load_hf_weights_compat(actor_weights, vllm_model):
    from vllm.attention.layer import Attention
    from vllm.model_executor.layers.quantization.base_config import QuantizeMethodBase
    from vllm.model_executor.model_loader.utils import set_default_torch_dtype

    act_dtype = next(vllm_model.parameters()).dtype
    with set_default_torch_dtype(act_dtype):
        if getattr(vllm_model.config, "tie_word_embeddings", False) and "lm_head.weight" in actor_weights:
            actor_weights = dict(actor_weights)
