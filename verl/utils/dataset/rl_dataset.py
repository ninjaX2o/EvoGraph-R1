...
from verl.utils.multimodal import (
    compute_position_ids as compute_multimodal_position_ids,
...
class ToolRLDataset(RLHFDataset):
    ...
    def __getitem__(self, item):
        ...
        image_grid_thw = None
        if self.processor is not None and self.image_key in row_dict:
            images = normalize_images(row_dict.pop(self.image_key))
            if images:
                ...
                prompt_with_chat_template, image_grid_thw = encode_prompt_images(
        ...
        row_dict['input_ids'] = input_ids[0]
        row_dict['attention_mask'] = attention_mask[0]
        row_dict['position_ids'] = position_ids if position_ids.ndim == 2 and position_ids.shape[0] == 3 else position_ids[0]
