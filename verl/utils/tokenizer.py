...
def hf_processor(name_or_path, **kwargs):
    """Create a Hugging Face processor when the model provides one.

    Text-only models generally do not ship processor files; returning None keeps
    the caller path compatible with both text and multimodal checkpoints.
    """
    from transformers import AutoProcessor

    try:
        return AutoProcessor.from_pretrained(name_or_path, **kwargs)
