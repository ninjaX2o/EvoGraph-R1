...
def _with_openrouter_provider(kwargs: dict, *, base_url: str | None, model: str) -> dict:
    resolved_base_url = (base_url or os.getenv("OPENAI_BASE_URL") or "").lower()
    if "openrouter.ai" not in resolved_base_url:
        return kwargs
    if not str(model).startswith("openai/"):
        return kwargs

    provider_order = os.getenv("OPENROUTER_PROVIDER_ORDER", "Azure")
    order = [item.strip() for item in provider_order.split(",") if item.strip()]
    if not order:
        return kwargs
    ...
    provider.setdefault("order", order)
...
def _is_openrouter_base_url(base_url: str | None) -> bool:
    resolved_base_url = (base_url or os.getenv("OPENAI_BASE_URL") or "").lower()
    return "openrouter.ai" in resolved_base_url


def _openai_async_client(base_url: str | None):
    openrouter_proxy = os.getenv("OPENROUTER_PROXY")
    if _is_openrouter_base_url(base_url) and openrouter_proxy:
        return AsyncOpenAI(
            base_url=base_url,
    ...
    return AsyncOpenAI() if base_url is None else AsyncOpenAI(base_url=base_url)


def _openai_sync_client(base_url: str | None):
    openrouter_proxy = os.getenv("OPENROUTER_PROXY")
    if _is_openrouter_base_url(base_url) and openrouter_proxy:
        return OpenAI(
            base_url=base_url,
    ...
    return OpenAI() if base_url is None else OpenAI(base_url=base_url)
...
async def openai_complete_if_cache(
    ...
    **kwargs) -> str:
    ...
    openai_async_client = _openai_async_client(base_url)
    kwargs.pop("hashing_kv", None)
    kwargs.pop("keyword_extraction", None)
    kwargs = _with_openrouter_provider(kwargs, base_url=base_url, model=model)
    messages = []
    if system_prompt:
...
async def zhipu_complete_if_cache(
    ...
    **kwargs,
    ...
    kwargs = _with_openrouter_provider(kwargs, base_url=None, model=model)
