...
class Tool(ABC):
    ...
    def validate_args(self, args: Dict) -> Tuple[bool, str]:
        ...
        schema = self.parameters or {"type": "object", "properties": {}, "required": []}
        ...
        # Type check provided fields if schema specifies them
        for key, val in args.items():
            if key in properties:
                prop_schema = properties[key]
                expected_type = prop_schema.get("type")
                if expected_type and not self._check_type(val, expected_type):
                    return False, f"Parameter '{key}' expects type '{expected_type}'"
                # Support oneOf: accept if value matches any listed type
                if not expected_type and "oneOf" in prop_schema:
                    one_of_types = [s.get("type") for s in prop_schema["oneOf"] if s.get("type")]
                    if one_of_types and not any(self._check_type(val, t) for t in one_of_types):
                        return False, f"Parameter '{key}' expects one of types: {one_of_types}"
